...
class ClientCreator(object):
    ...
    def create_client(
        ...
        self._register_s3_events(service_client, endpoint_bridge, endpoint_url, client_config, scoped_config)
    ...
    def _create_client_class(self, service_name, service_model):
        ...
        self._event_emitter.emit(
            'creating-client-class.%s' % service_id, class_attributes=class_attributes, base_classes=bases
        )
    ...
    def _register_retries(self, client):
        ...
        client.meta.events.register('needs-retry.%s' % service_event_name, handler, unique_id=unique_id)
    ...
    def _register_s3_events(self, client, endpoint_bridge, endpoint_url, client_config, scoped_config):
        ...
        self._set_s3_addressing_style(endpoint_url, client.meta.config.s3, client.meta.events, client.meta.partition)
        ...
        if self._is_s3_accelerate(endpoint_url, client.meta.config.s3):
            ...
            client.meta.events.register_first('before-sign.s3', switch_host_s3_accelerate)

        self._set_s3_presign_signature_version(client.meta, client_config, scoped_config)

    def _set_s3_addressing_style(self, endpoint_url, s3_config, event_emitter, partition):
        if s3_config is None:
            s3_config = {}

        addressing_style = self._get_s3_addressing_style(endpoint_url, s3_config)
        handler = self._get_s3_addressing_handler(endpoint_url, s3_config, addressing_style, partition)
    ...
    def _get_s3_addressing_handler(self, endpoint_url, s3_config, addressing_style, partition):
    ...
    def _get_client_args(
        ...
        args_creator = ClientArgsCreator(
            self._event_emitter, self._user_agent, self._response_parser_factory, self._loader, self._exceptions_factory
        )
    ...
    def _create_api_method(self, py_operation_name, operation_name, service_model):
        def _api_call(self, *args, **kwargs):
            ...
            # 1 argument.
            if args:
                raise TypeError("%s() only accepts keyword arguments." % py_operation_name)
...
class ClientEndpointBridge(object):
    ...
    def _create_endpoint(self, resolved, service_name, region_name, endpoint_url, is_secure):
        ...
        if endpoint_url is None:
            if self._is_s3_dualstack_mode(service_name):
                ...
                endpoint_url = self._make_url(hostname, is_secure, resolved.get('protocols', []))
    ...
    def _create_dualstack_endpoint(self, service_name, region_name, dns_suffix, is_secure):
    ...
    def _assume_endpoint(self, service_name, region_name, endpoint_url, is_secure):
        ...
        logger.debug('Assuming an endpoint for %s, %s: %s', service_name, region_name, endpoint_url)
    ...
    def _create_result(
        self, service_name, region_name, signing_region, signing_name, endpoint_url, signature_version, metadata
    ):
        return {
            'service_name': service_name,
            'region_name': region_name,
        ...
        if 'signatureVersions' in resolved:
            ...
            for known in potential_versions:
                if known in AUTH_TYPE_MAPS:
                    return known
        raise UnknownSignatureVersionError(signature_version=resolved.get('signatureVersions'))
...
class BaseClient(object):
    ...
    def __init__(
        ...
        self.meta = ClientMeta(
            event_emitter, self._client_config, endpoint.host, service_model, self._PY_TO_OP_NAME, partition
        )
        self._exceptions_factory = exceptions_factory
    ...
    def _register_handlers(self):
        ...
        self.meta.events.register('request-created.%s' % service_id, self._request_signer.handler)
    ...
    def _make_api_call(self, operation_name, api_params):
        ...
        history_recorder.record(
            'API_CALL', {'service': service_name, 'operation': operation_name, 'params': api_params}
        )
        ...
        handler, event_response = self.meta.events.emit_until_response(
            'before-call.{service_id}.{operation_name}'.format(service_id=service_id, operation_name=operation_name),
        ...
        self.meta.events.emit(
            'after-call.{service_id}.{operation_name}'.format(service_id=service_id, operation_name=operation_name),
            http_response=http,
    ...
    def _convert_to_request_dict(self, api_params, operation_model, context=None):
        ...
        prepare_request_dict(
            request_dict, endpoint_url=self._endpoint.host, user_agent=self._client_config.user_agent, context=context
        )
        return request_dict
        ...
        if 'page_config' not in self._cache:
            try:
                page_config = self._loader.load_service_model(
                    self._service_model.service_name, 'paginators-1', self._service_model.api_version
                )['pagination']
        ...
        if 'waiter_config' not in self._cache:
            try:
                waiter_config = self._loader.load_service_model(
                    self._service_model.service_name, 'waiters-2', self._service_model.api_version
                )
